...
document
  ...
  .addEventListener("submit", async function (e) {
    ...
    try {
      // Gửi yêu cầu POST đến server tại route '/api/v1/submit' với dữ liệu JSON
      const response = await fetch("/api/v1/submit", {
        ...
        body: JSON.stringify({ name }), // Chuyển đổi đối tượng thành chuỗi JSON
      });
...
document
  ...
  .addEventListener("submit", async function (e) {
    ...
    try {
      // Gửi yêu cầu POST đến server tại route '/api/v1/bmi' với dữ liệu JSON
